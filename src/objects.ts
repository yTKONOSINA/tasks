...
/*
The purpose of this chapter is for you to write functions that do not mutate the arguments to your function for object arguments; instead you need to make a new copy. For simple objects you can make a shallow copy, like this:
Using the spread operator makes a *shallow* copy of any object argument.
...
export function toShortForm(question: Question): string {
    return String(question.id) + ": " + String(question.name.slice(0, 10));
}
...
export function toMarkdown(question: Question): string {
    let ans = "# " + String(question.name) + "\n" + String(question.body);
    if (question.type === "multiple_choice_question") {
        ans += "\n";
        const opt = question.options.map((option) => `- ${option}`);
        const str = opt.join("\n");
        ans += str;
    }
    return ans;
...
export function duplicateQuestion(id: number, oldQuestion: Question): Question {
    return {
        ...oldQuestion,
        id,
        name: `Copy of ` + String(oldQuestion.name),
        published: false,
    };
}
